--@name Dingus Bazooka v6.6 - Base Logic
--@author vertihluy
--@shared
...
if SERVER then
    ...
    hook.add("think", "DingusBazookaThink", function()
        ...
        if maxwellBomb and isValid(maxwellBomb.holo) then
            ...
            local accelFrac = math.clamp((elapsed - MAXWELL_PRIME_TIME) / MAXWELL_ACCEL_TIME, 0, 1)
            local danceSpeed = 1 + accelFrac * 2.5
            ...
            local pulseFrac = math.clamp((elapsed - pulseStart) / (MAXWELL_ACCEL_TIME + 1.1), 0, 1)
            local pulseWave = math.sin(elapsed * (8 + pulseFrac * 8))
            local pulseOffset
            if pulseWave >= 0 then
                pulseOffset = pulseWave * (0.12 + pulseFrac * 0.42)
            else
                pulseOffset = pulseWave * (0.05 + pulseFrac * 0.11)
            end
            local pulseScale = 1 + pulseFrac * 0.18 + pulseOffset
